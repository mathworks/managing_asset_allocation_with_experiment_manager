function [new_weights] = equalweight(weights,returns,lambda)
    ...
    HHMixObjFun = @(x) x'*p.AssetCovar*x + lambda*(x'*x);
    % Solution that accounts for risk and HH diversification
    new_weights = estimateCustomObjectivePortfolio(p,HHMixObjFun);
end