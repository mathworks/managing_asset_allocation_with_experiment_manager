function [new_weights] = erc(weights,returns,lambda)
    ...
    ERCMixObjFun = @(x) x'*p.AssetCovar*x - lambda*sum(log(x));
    % Solution that accounts for risk and ERC diversification
    new_weights = estimateCustomObjectivePortfolio(p,ERCMixObjFun);
end